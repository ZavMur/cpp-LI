...
int main(int argc, char const *argv[])
{
    ...
    if (nota >= 96 && nota <= 100) {
        cout << "obtubiste una S (A+++). Corresponde a un trabajo o examen perfecto. Trabajo muy bueno y sin errores. Felicidades.";
    }

    if (nota >= 95 && nota <= 91) {
        cout << "obtubiste una AD. Corresponde a un trabajo o examen bueno con pocas faltas. Esfuersate un poco mas y alcanzaras la excelencia.)";
    }

    if (nota >= 90 && nota <= 86) {
        cout << "obtubiste una A. Realización con solo errores menores. continua así vas por buen camino.)";
    }

    if (nota >= 85 && nota <= 81) {
        cout << "obtubiste una A-. Ya estas sobre la media. Tu camino a la exelencia inicia aquí. )";
    }
    if (nota >= 80 && nota <= 76) {
    ...
    if (nota >= 50 && nota <= 46) {
        cout << "obtubiste una D+. casi pero reprobado. Es el mínimo aprobatorio en España y Venezuela. )";
    }
    if (nota >= 45 && nota <= 41) {
